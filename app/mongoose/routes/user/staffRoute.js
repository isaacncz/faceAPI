...
router.post('/register', (req, res) => {
  const { name, email, password, password2, id, department, age } = req.body;
  let errors = [];
          ...
          bcrypt.hash(newUser.password, salt, (err, hash) => {
            ...
            newUser
              ...
              .then(() => {
                // host = req.get('host');
                // link =
                //   process.env.STAFF_BACKEND_URL + '/user/verify?id=' + randKey;
                // console.log(link)
                // var mail = {
                ...
                //   html:
                //     '<p>You successfully registered an account at ' +
                //     host +
...
router.get('/verify', function (req, res) {
  ...
  if (req.get('host') == req.get('host')) {
    ...
    User.find(condition)
      ...
      .then((data) => {
        if (data.n === 1) {
          res.redirect(process.env.STAFF_FRONTEND_URL + '/verify');
        } else {
...
router.post('/forgotPassword', async (req, res) => {
  ...
  try {
    ...
    link =
      process.env.STAFF_BACKEND_URL +
      `/user/reset/?resetPasswordToken=${token}`;
    if (!data) {
      res.status(404).send({
